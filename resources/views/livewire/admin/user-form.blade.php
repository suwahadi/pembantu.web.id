<div class="max-w-3xl mx-auto space-y-8">
    ...
    <form wire:submit.prevent="save" class="space-y-8">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900/70 md:p-8">
            ...
            @if($lockRole)
                @php
                    $selected = collect($roleOptions)->firstWhere('id', $fixedRoleId);
                    $selectedLabel = $selected['label'] ?? 'Agency Partner';
                @endphp
                <div class="rounded-xl border border-gray-200 bg-white px-5 py-4 text-sm font-medium text-gray-700 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200">
                    {{ $selectedLabel }}
                </div>
            @else
        ...
        <div class="flex items-center justify-between mt-8">
            <a href="{{ route($redirectRoute ?: 'admin.users.index') }}" class="px-4 py-2 rounded-lg border border-primary-600 bg-gray-900 text-white hover:bg-gray-800 dark:bg-white dark:text-gray-900 dark:border-gray-300 dark:hover:bg-gray-100 transition-colors">Kembali ke Daftar</a>
