...
>
    ...
    <div class="space-y-6">
        <!-- Payment Content (Full Width) -->
        <div>
            @if($this->paymentDetails && isset($this->paymentDetails['payment_success']) && $this->paymentDetails['payment_success'])
                {{-- SUCCESS STATE --}}
    ...
    $status = strtolower($this->paymentDetails['transaction_status'] ?? 'pending');
    
    if ($status === 'settlement' || $status === 'capture' || $status === '100') {
        // Success states - Green
        $statusClass = 'text-emerald-800 dark:text-emerald-200';
        $statusBgClass = 'bg-emerald-100 dark:bg-emerald-900/30';
        $statusBorderClass = 'border-emerald-200 dark:border-emerald-800';
        $boxClass = 'border-emerald-200 bg-emerald-50 dark:border-emerald-900/40 dark:bg-emerald-900/20';
    } elseif (in_array($status, ['pending', '201', '202', '407', '408'])) {
        // Pending states - Yellow
        $statusClass = 'text-amber-800 dark:text-amber-200';
        $statusBgClass = 'bg-amber-100 dark:bg-amber-900/30';
        $statusBorderClass = 'border-amber-200 dark:border-amber-800';
        $boxClass = 'border-amber-200 bg-amber-50 dark:border-amber-900/40 dark:bg-amber-900/20';
    } else {
        // Failed states - Red (expire, cancel, deny, failure, etc.)
        $statusClass = 'text-red-800 dark:text-red-200';
        $statusBgClass = 'bg-red-100 dark:bg-red-900/30';
        $statusBorderClass = 'border-red-200 dark:border-red-800';
        $boxClass = 'border-red-200 bg-red-50 dark:border-red-900/40 dark:bg-red-900/20';
    }
@endphp

                <div class="rounded-2xl {{ $boxClass }} p-5 shadow-sm">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div class="text-sm font-semibold text-gray-900 dark:text-white">Instruksi Pembayaran</div>
                        <div class="inline-flex items-center rounded-full border {{ $statusBorderClass }} {{ $statusBgClass }} px-3 py-1 text-sm font-semibold {{ $statusClass }}">
                            STATUS:&nbsp;<span class="font-semibold">{{ Str::upper($this->paymentDetails['transaction_status'] ?? 'pending') }}</span>
                        </div>
                    </div>

                    <div class="mt-6 rounded-2xl border {{ $statusBorderClass }} bg-white p-5 dark:bg-gray-900">
        ...
        <!-- Order Summary & Guide (Below Payment Content) -->
